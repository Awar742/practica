...
public class Controller {
    ...
    public void setItems(ActionEvent event) {
        ObservableList<File> ol =a.getLoc(folderLocationField.getText());
        listView.setItems(FXCollections.observableArrayList(b.check(ol, timeField.getText())));
        }

    public void showSize(ActionEvent event){
        Location l = new Location();
        ObservableList<File> ol =l.getLoc(folderLocationField.getText());
    ...
    public void showAmount(ActionEvent event) {
        Location a = new Location();
        ObservableList<File> l =a.getLoc(folderLocationField.getText());
        int count=0;
        if (l != null || l.size() != 0) {
            for (File s : l) {
                count++;
                ...
                if (selectedIdx != -1) {
                    ...
                    final int newSelectedIdx =
                            (selectedIdx == listView.getItems().size() - 1)
                                    ? selectedIdx - 1
                                    : selectedIdx;
                    l.getFele(selectedIdx, folderLocationField.getText()).delete();
